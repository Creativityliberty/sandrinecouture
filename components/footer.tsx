      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 text-left mb-16">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-full border border-black/10 overflow-hidden bg-white shadow-sm p-1">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm md:text-xs text-gray-700 font-medium list-none p-0">
              {SITE_CONFIG.seo.sectors.map(s => (
                <li key={s.slug}><Link href={`/devis?secteur=${s.slug}`} className="hover:text-black transition-colors no-underline">{s.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm md:text-xs text-gray-700 font-medium list-none p-0">
              {SITE_CONFIG.seo.products.map(p => (
                <li key={p.slug}><Link href={`/devis?produit=${p.slug}`} className="hover:text-black transition-colors no-underline">{p.name}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm md:text-xs text-gray-700 font-medium list-none p-0">
              {SITE_CONFIG.seo.regions.map(r => (
                <li key={r.slug}><Link href={`/#${r.slug}`} className="hover:text-black transition-colors font-bold no-underline">{r.name}</Link></li>
              ))}
              <li className="pt-2 border-t border-black/10 mt-3">
                <div className="flex flex-wrap gap-2">
                  {SITE_CONFIG.seo.cities.map(c => (
                    <Link key={c} href={`/#broderie-${c.toLowerCase()}`} className="text-[10px] md:text-[9px] hover:text-black no-underline uppercase tracking-tighter text-gray-600">{c}</Link>
                  ))}
                </div>
              </li>
            </ul>
        ...
        <div className="pt-8 border-t border-black/10 flex flex-col md:flex-row justify-between items-center gap-4 text-xs md:text-[9px] font-bold text-gray-600 uppercase tracking-widest">
          <div className="flex gap-6">
            <Link href="/mentions-legales" className="hover:text-black no-underline">Mentions Légales</Link>
            <Link href="/confidentialite" className="hover:text-black no-underline">Confidentialité</Link>
            <Link href="/cgv" className="hover:text-black no-underline">CGV</Link>
          </div>
