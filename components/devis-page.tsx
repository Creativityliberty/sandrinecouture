    ...
    <div className="pt-32 pb-24 px-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-4 mb-12">
          {(["entreprise", "particulier"] as const).map((type) => (
            <button
              ...
              className={`flex-1 py-6 px-4 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-3 bg-white cursor-pointer ${
                formType === type
                  ? "border-primary bg-primary/5 shadow-xl scale-105 z-10"
                  : "border-black/5 opacity-50 grayscale hover:grayscale-0 hover:opacity-100"
              }`}
            >
              {type === "entreprise" ? (
        ...
        <form
          ...
          className="glass p-8 md:p-16 rounded-[3rem] space-y-10"
        >
          {/* SECTION 1: CONTACT */}
          <div className="space-y-6">
            <h3 className="text-xl font-black italic uppercase tracking-tight flex items-center gap-3">
              <span className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center text-xs not-italic">
                1
              </span>
              Vos coordonnées
            </h3>
                  ...
                  {[
                    {
                      ...
                      desc: "Prénom, slogan, initiales",
                    },
                    {
                      id: "simple",
                      label: "Logo Simple",
                      desc: "1 à 3 couleurs, formes nettes",
                    },
                    {
                      id: "complex",
                      label: "Logo Complexe",
                      desc: "Beaucoup de couleurs ou détails",
                    },
                  ].map((opt) => (
                    <label
                      key={opt.id}
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setFormData({ ...formData, complexity: opt.id })
                        }
                      />
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${formData.complexity === opt.id ? "border-primary" : "border-gray-300"}`}
                      >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {placements.map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, placement: p.id })
                      }
          ...
          <div className="pt-6">
            <Button
              variant="whatsapp"
              className="w-full h-20 text-xs sm:text-sm uppercase tracking-[0.2em] font-black shadow-2xl shadow-green-500/20 rounded-full"
              type="submit"
            >
              <MessageCircle className="mr-3 w-6 h-6" /> Envoyer ma demande
              WhatsApp
            </Button>
            <p className="text-center text-[10px] uppercase font-black tracking-[0.3em] opacity-40 mt-6 flex items-center justify-center gap-3">
              ...
              Sandrine traite votre demande personnellement sous 24h
            </p>
          </div>
        </form>
      </div>
...
export function DevisPage() {
  return (
    <Suspense
      fallback={
        <div className="pt-32 text-center font-bold">Chargement...</div>
      }
    >
      <DevisForm />
    </Suspense>
